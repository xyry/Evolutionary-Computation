#!/usr/bin/env python
# _*_coding:utf-8 _*_
# @Time    : 2019/11/14 19:52
# @Author  : YPL
# @FileName: evo_1_n=2.py
# @Software: PyCharm
...
class GeneticAlgorithm(object):
    ...
    def transformDNA(self, population):
        ...
        if population.ndim==2:
            ...
            p2=population[:,10:20]
            population_decimal = (
                    (p1.dot(np.power(10, np.arange(self.DNA_size/2)[::-1])) / np.power(10, self.DNA_size/2)) *
                    (self.x_bounder[1] - self.x_bounder[0]) - 10
            )
            c.append(population_decimal)
            population_decimal = (
                    (p2.dot(np.power(10, np.arange(self.DNA_size/2)[::-1])) / np.power(10, self.DNA_size/2)) *
                    (self.x_bounder[1] - self.x_bounder[0]) - 10)
            c.append(population_decimal)
            return c
...
def main():
    ga = GeneticAlgorithm(cross_rate=0.8, mutation_rate=0.01, n_population=300, n_iterations=2000, DNA_size=18)
    ga.evolution()
