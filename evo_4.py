...
class GeneticAlgorithm(object):
    ...
    def transformDNA(self, population):
        ...
        if population.ndim==2:
            ...
            c.append(population_decimal)
            population_decimal = (
                    (p4.dot(np.power(10, np.arange(self.DNA_size/self.n)[::-1])) / np.power(10, self.DNA_size/self.n)) *
                    (self.x_bounder[1] - self.x_bounder[0]) - 10)
            c.append(population_decimal)
    ...
    def evolution(self):
        ...
        for i in range(self.n_iterations):
            ...
            if i % 100 == 0:
                print(u'第%-4d次进化后, 基因最好的个体是: %s, \n其适应度是: -%.12f' % (i, best_person,
                                                                                        f(self.transformDNA(
                                                                                            best_person))))
...
def main():
    ga = GeneticAlgorithm(cross_rate=0.9, mutation_rate=0.01, n_population=200, n_iterations=2000, DNA_size=27)
    ga.evolution()
